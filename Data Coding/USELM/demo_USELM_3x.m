...
% %     
    L=laplacian(options,elmModel2.Embed);
    ...
    %paras.Kernel = 'sinusoid';
    paras2.lambda=0.1;
    elmModel3=uselm(elmModel2.Embed,elmModel2.Embed_test,L,paras);
    

    % model = train(y,sparse(elmModel.Embed),[,'s 0']);
    % [predicted_label] = predict(y_test, sparse(elmModel.Embed_test), model,[, 's 0']);
    disp('Train SVM');
    model2 = svmtrain(y_train, elmModel3.Embed, [, '-t 0','-h 0']);
    [predicted_label,accuracy1, decision_values] = svmpredict(y_test,elmModel3.Embed_test, model2);
    
    acc_USELM_linear(i) = accuracy1(1);
    
    model2 = svmtrain(y_train, elmModel3.Embed, [, '-t 2','-h 0']);
    [predicted_label,accuracy1, decision_values] = svmpredict(y_test,elmModel3.Embed_test, model2);
